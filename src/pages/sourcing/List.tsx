...
export default function SourcingList() {
  ...
  return (
    <>
      <PageHeader title="Sourcing & quotations" subtitle="Approved requisitions ready for the market. The sourcing method is set by the SOP value tier — direct purchase, 3 quotations, closed bid or open tender." />
      <Card padded={false}>
        {queue.length === 0 ? <div className="p-5"><EmptyState title="Nothing to source" body="Approved requisitions will land here automatically." icon={<Search size={22} />} /></div> : (
          <div className="overflow-x-auto scrollbar-thin">
            <table className="w-full min-w-[900px] text-[13px]">
              ...
              <tbody>{queue.map((p) => {
                const tier = tierForPR(p, settings).tier
                const need = Math.max(tier?.minQuotations ?? 0, 1)
                ...
                return (
                  <tr key={p.id} className="cursor-pointer hover:bg-surface-muted" onClick={() => nav(`/sourcing/${p.id}`)}>
                    ...
                    <td className="table-td">{tier ? <MethodBadge method={tier.method} /> : '—'}{p.sourcing.exception && <div className="mt-0.5 text-[11px] text-accent-700">Sole-source / emergency</div>}</td>
